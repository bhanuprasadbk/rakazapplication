...
export class SensorGroupComponent implements OnInit {
  ...
  selectedSensor: any;
  //ag-grid
  pageSize = 1; // or bind this from user selection
  pageSizeSelector = [1, 20, 50, 100];
  rowHeight = 40; // default row height (or whatever you use)
  gridApi!: GridApi<any>;
  sensorGroupsList: any[] = [];
  columnDefs: ColDef[] = [
    { field: 'sensor_parameter', headerName: 'Parameter',hide:true },
  ...
  getSensorGroups() {
    this.customerAdminService.getSensorGroups().subscribe((res: any) => {
      console.log(res);
